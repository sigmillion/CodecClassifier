...
public:
   ...
   graph_search_metric() : dist(100000000), label(255) {}
   ~graph_search_metric() {}
};
   ...
   graph_search_metric recursive_graph_search(std::vector<unsigned char> & x, codeword & c, int dist) {
      ...
      if(d != dec.end()) { // Found it in the dictionary
         if(d->second.label == 255) {
            ...
            fprintf(stderr,"%10d ... Returning %s\n",dist,strng.c_str());
            dist = 100000000; // Make the distance huge so that this return path is not taken
         } else {
            fprintf(stderr,"%10d ... Found label %d\n",dist,d->second.label);
            //printf("Hello");
         }
      ...
      fprintf(stderr,"Setting label = %d, dist = %d\n",bestgsm.label,bestgsm.dist);
